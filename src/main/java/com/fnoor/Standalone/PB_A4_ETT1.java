...
public class PB_A4_ETT1 {
    ...
    public static void customTarget(String testId) throws InterruptedException {
        driver.get("https://test.engagingnetworks.app/page/11068/action/1?mode=DEMO");

        fields.selectTitle("Miss");
        fields.setFirstname("Custom");
        fields.setLastname("Target");
        ...
        Assert.assertTrue("Urls are not the same", driver.getCurrentUrl().equals("https://test.engagingnetworks.app/page/11068/action/2"));
        ...
        fields.validateETTTargetMessage("Custom Target");
        fields.submit();

        String myurlfinalUrl = driver.getCurrentUrl();
        Assert.assertTrue("Urls are not the same", myurlfinalUrl.equals("https://test.engagingnetworks.app/page/11068/action/3"));
        //		Get the details from the third page and Verify the fields
        String bodytext = driver.findElement(By.tagName("body")).getText();
        Assert.assertTrue("First name missing from last page ", bodytext.contains("Custom"));
        Assert.assertTrue("Last name missing from last page", bodytext.contains("Target"));
